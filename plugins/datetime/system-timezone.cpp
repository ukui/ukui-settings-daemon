...
SystemTimezone* SystemTimezone::mSystemTimezone = nullptr;

static const char *files_to_check[CHECK_NB] = {
        ETC_TIMEZONE,
        ETC_TIMEZONE_MAJ,
        ETC_SYSCONFIG_CLOCK,
        ETC_CONF_D_CLOCK,
...
void system_timezone_monitor_changed (GFileMonitor *handle,
                                             GFile *file,
                                             GFile *other_file,
                                             GFileMonitorEvent event,
                                             gpointer user_data);

SystemTimezone::SystemTimezone()/*(QObject* parent):QObject(parent)*/
{
    int i;

    mSystemTimezone->tz = NULL;
    mSystemTimezone->env_tz = NULL;
    for (i = 0; i < CHECK_NB; i++)
            mSystemTimezone->monitors[i] = NULL;
...
{
    if(mSystemTimezone)
        delete mSystemTimezone;
}

SystemTimezone *SystemTimezone::SystemTimezoneNew (void)
{
    if(nullptr == mSystemTimezone)
        mSystemTimezone = new SystemTimezone();
...
void SystemTimezone::system_timezone_constructor ()
{
        mSystemTimezone->tz = system_timezone_find ();
        mSystemTimezone->env_tz = g_strdup (getenv ("TZ"));
        ...
        for (i = 0; i < CHECK_NB; i++) {
                ...
                if (parent_type == G_FILE_TYPE_DIRECTORY)
                        mSystemTimezone->monitors[i] = g_file_monitor_file (file,
                                                                 G_FILE_MONITOR_NONE,
                                                                 NULL, NULL);
                g_object_unref (file);

                //下面这一处是为了不报语法错误，而加的强制类型转换
                if (mSystemTimezone->monitors[i]){
                    //connect((QObject*)mSystemTimezone->monitors[i],SLOT(monitorChanged(GFileMonitorEvent)),
                    //        this,SLOT(system_timezone_monitor_changed(GFileMonitorEvent)));
                    //GFileMonitor有自己的changed信号，与本类new的changed不同
                    g_signal_connect (G_OBJECT (mSystemTimezone->monitors[i]),
                                      "changed",
...
void SystemTimezone::SystemTimezoneStop ()
{
        int i;

        if (mSystemTimezone->tz) {
                g_free (mSystemTimezone->tz);
                mSystemTimezone->tz = NULL;
        }

        if (mSystemTimezone->env_tz) {
                g_free (mSystemTimezone->env_tz);
                mSystemTimezone->env_tz = NULL;
        }

        for (i = 0; i < CHECK_NB; i++) {
                if (mSystemTimezone->monitors[i])
                        g_object_unref (mSystemTimezone->monitors[i]);
                mSystemTimezone->monitors[i] = NULL;
        }
}

void
SystemTimezone::system_timezone_monitor_changed (GFileMonitor *handle,
                                 GFile *file,
                                 GFile *other_file,
                                 GFileMonitorEvent event,
                                 gpointer user_data)
{
    ...
    g_assert (mSystemTimezone->tz != NULL && new_tz != NULL);

    if (strcmp (mSystemTimezone->tz, new_tz) != 0) {
            g_free (mSystemTimezone->tz);
            mSystemTimezone->tz = new_tz;
            //TODO...
            //emit monitorChanged(mSystemTimezone->tz);
